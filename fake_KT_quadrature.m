function [integral, weights, coefficients] =...
    fake_KT_quadrature(nodes, values, alpha, degree)

% Scaled KTI & KTL (Kosloff Tal-Ezer) quadrature formula.
%
...
% integral               : approximation of the exact value of the integral
%
% OUTPUT (M = N):
% weights [M+1 x 1]      : M+1 interpolatory quadrature weights
% coefficients [N+1 x 1] : values (INPUT)
% integral               : approximation of the exact value of the integral
    ...
    % Interpolation/Quadrature matrix with Chebyshev polynomials
    NDCT = zeros(M+1,N+1);
    for i = 1:N+1
    ...
    if N == M
        ...
        coefficients = values;
        % not scaled approximation of the integral
        integral = values * weights;
    elseif N > M
        disp("The cardinality of the basis exceeds the number of nodes!")
        return
    else
        disp('least-squares quadrature')
        ...
        weights = intcosw;
        % not scaled approximation of the integral
        integral = weights * coefficients;
        
    end
    % scaled approximation of the integral
    integral = integral * scale/2;
